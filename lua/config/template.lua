...
template.load = function()
	...
	local home = vim.env.NVIM_BEGINNER

	if filetype == "" then
		filetype = vim.fn.expand "%"
	end

	local fname = string.format("%s/nvim/templates/%s/skel.%s", home,  filetype, extension)
...
template.yaml = function()
	...
	local filetype  = vim.bo.filetype
	local home = vim.env.NVIM_BEGINNER
	local fname = string.format("%s/nvim/templates/%s/cheetah.yml",home, filetype)
