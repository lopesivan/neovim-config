...
nmap {'<leader><CR>', 'make', 'build manager'}
nmap {'s<CR>', 'show', 'Show file'}
nmap {'c<CR>', 'dispatch', 'make clean'}
...
local function code_block()
	...
	vim.api.nvim_buf_set_text(0, range[1], 0, range[1], 0, {"\\begin{flushleft}",""})

	if range[2] >= vim.api.nvim_buf_line_count(0) then
		vim.api.nvim_buf_set_lines(0, range[2], range[2], true, {""})
	end

	vim.api.nvim_buf_set_text(0, range[2], 0, range[2], 0, {"\\end{flushleft}",""})
end
